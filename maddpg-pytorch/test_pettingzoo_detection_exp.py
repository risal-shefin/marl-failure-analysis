                ...
                if i not in fault_first_detected:
                    ...
                    if len(prev_faults) > 0:
                        for f, tf in prev_faults:
                            ...
                            #     values_over_time = [frob_norms_matrix_history[tau][i][agent] for tau in range(tf, cnt + 1) if tau < len(frob_norms_matrix_history)]
                            #     if len(values_over_time) > 0:
...
def plot_action_influences(action_influences_matrix_history, attacked_steps, atk_agent_id, logdir):
    """
    ...
    T = len(action_influences_matrix_history)     # number of timesteps
    
    # Create N subplots (one for each agent showing influences on it)
    max_per_row = 3
    rows = math.ceil(N / max_per_row)
    cols = min(N, max_per_row)
    ...
    for i in range(N):
        ...
        for j in range(N):
            influence_series = [action_influences_matrix_history[t][i][j] for t in range(T)]
            timesteps = range(T)
...
def plot_frob_norm_influences(frob_norms_matrix_history, attacked_steps, atk_agent_id, logdir):
    """
    ...
    T = len(frob_norms_matrix_history)     # number of timesteps
    ...
    fig.suptitle(f'Frobenius Norm Influences: ||∂²Q_i/(∂obs_i∂obs_j)||_F (Attacked Agent ID: {atk_agent_id})', fontsize=16, y=0.96)
    ...
    for i in range(N):
        ...
        for j in range(N):
            influence_series = [frob_norms_matrix_history[t][i][j] for t in range(T)]
            timesteps = range(T)
...
def plot_second_order_action_influences(second_order_action_influences_history, attacked_steps, atk_agent_id, logdir):
    """
    ...
    T = len(second_order_action_influences_history)     # number of timesteps
    ...
    for i in range(N):
        ...
        for j in range(N):
            influence_series = [second_order_action_influences_history[t][i][j] for t in range(T)]
            timesteps = range(T)
...
def plot_observation_influences(observation_influences_matrix_history, attacked_steps, atk_agent_id, logdir):
    """
    ...
    T = len(observation_influences_matrix_history)     # number of timesteps
    ...
    for i in range(N):
        ...
        for j in range(N):
            influence_series = [observation_influences_matrix_history[t][i][j] for t in range(T)]
            timesteps = range(T)
...
def run(config):
    ...
    try:
        ...
        if config.env_id == "simple_spread_v3":
            env = env_func.parallel_env(continuous_actions= not maddpg.discrete_action, render_mode='rgb_array', N=3)
        else:
    ...
    plot_frob_norm_influences(frob_norms_matrix_history, attacked_steps, attacked_agent_id, logdir)
    plot_sec_dir_derivatives(sec_dir_derivatives_normal, sec_dir_derivatives_atk, attacked_steps, attacked_agent_id, logdir)
    plot_action_influences(action_influences_matrix_history, attacked_steps, attacked_agent_id, logdir)
    ...
    plot_second_order_action_influences(second_order_action_influences_history, attacked_steps, attacked_agent_id, logdir)
    ...
    plot_observation_influences(observation_influences_matrix_history, attacked_steps, attacked_agent_id, logdir)
    ...
    plot_second_order_observation_influences(second_order_observation_influences_history, attacked_steps, attacked_agent_id, logdir)
