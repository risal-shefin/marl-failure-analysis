...
def get_episode_data(env, maddpg, config, logdir, ref_vals, ref_std_devs, detection_method='mean_std', do_attack=False, atk_agent_id=-1, seed=None):
    ...
    exceed_counts = [0 for _ in range(maddpg.nagents)]
    ...
    print("")
    print(f"Anomaly exceed counts in 1~15 steps: {exceed_counts}")
...
def plot_results(results_attacked, attacked_steps, atk_agent_id, ref_vals, ref_std_devs, logdir, detection_method='mean_std'):
    ...
    fig.suptitle(f'Taylor Expansion Error | Attacked Agent ID: {atk_agent_id}', fontsize=16, y=0.95)
...
def plot_fault_timeline_action_influences(fault_timeline, action_influences_matrix_history, total_agents, logdir):
    ...
    k_top = 1  # Look for faulty agents in top-2 influencers
...
def plot_normal_scenario_action_influences_stacked(timesteps, agents, action_influences_matrix_history_normal, total_agents, logdir):
    """
...
def run(config):
    ...
    logdir = os.path.join(cwd, 'runs', f"{config.env_id}_{'discrete' if maddpg.discrete_action else 'continuous'}", f"{timestamp}_seed_{config.seed}")
    ...
    # Get timesteps and agents from the stacked plot and create comparison plot
