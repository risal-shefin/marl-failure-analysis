...
def plot_fault_timeline_action_influences(fault_timeline, action_influences_matrix_history, total_agents, logdir):
    ...
    for col, event in enumerate(fault_timeline):
        ...
        if col == 0:
            ...
            # Calculate action influence contributors for this fault event
            faulty_agent = event['agent']
            fault_timestep = event['t']
            
            # Get action influences at the fault timestep
        ...
        if len(contribs) == 0:
            ...
            if col == 0:
                ...
                ax.text(0.5, 0.5, 'No action influences',
                        ha='center', va='center', fontsize=10, style='italic')
        else:
            vals = np.array(list(contribs.values()), dtype=float)
            if vals.sum() > 0:
    ...
    fig.suptitle('Fault Detection Timeline and Action Influence Contributors',
                 fontsize=14, fontweight='bold', y=0.96)

    out_path = os.path.join(logdir, 'fault_timeline_action_influences.png')
    plt.savefig(out_path, dpi=300, bbox_inches='tight')
    plt.show()
    print(f"Saved action influences fault timeline plot to {out_path}")
