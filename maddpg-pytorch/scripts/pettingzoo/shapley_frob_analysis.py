#!/usr/bin/env python3
"""
Integrated Shapley Values and Frobenius Norm Analysis for Multi-Agent RL (PettingZoo Version)

This script combines Monte Carlo Shapley value computation with Frobenius norm analysis
to provide comprehensive agent influence and risk assessment. It includes:
...
5. GIF saving functionality for coalition rollouts

Features:
- Fixed episode counts: 100 for Shapley, 1 for Frobenius
- Outbound influence score: sum of frob[j][i] for all j != i
- Cascade Risk Index: max(0, -shapley_i) * I_i^out
- Comprehensive visualization with bar charts
- GIF creation for unique coalitions
...
def save_results(shapley_values, running_means_history, avg_frob_norms, 
                outbound_influence, cascade_risk, logdir, config):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Integrated Shapley Values and Frobenius Analysis for Multi-Agent RL (PettingZoo)')
    ...
    parser.add_argument("--save_gifs", action="store_true",
                        help="Save GIFs of coalition rollouts and analysis episodes")
