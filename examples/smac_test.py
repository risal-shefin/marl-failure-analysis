    ...
    while True:
        ...
        for agent_id in range(n_agents):
            if agent_id in coalition:
                ...
                action_array = np.array([[0]], dtype=np.int64)
...
def main():
    ...
    # Model restoration
    parser.add_argument("--restore_dir", type=str, default="/deac/csc/vanbastelaerGrp/guptd23/RL_Project/HARL/examples/results/pettingzoo_mpe/simple_spread_v3-discrete/happo/Latest_5/seed-00001-2025-08-15-22-58-50/models")
    parser.add_argument("--restore_reward", type=str, default="-75.127")
    ...
    render_val = runner.eval_envs.render()
    print(f"Render output type: {type(render_val)}, shape: {getattr(render_val, 'shape', 'N/A')}")
    
    exit("Exiting after render test")
    ...
    if args.save_gifs:
        ...
        try:
            ...
            test_obs, test_share_obs, test_avail = runner.eval_envs.reset(args.seed)
