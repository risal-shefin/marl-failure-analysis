...
def plot_frob_full_details(normal, first_attack, second_attack, third_attack,save_path,attack_type):
    ...
    agents = [0, 1, 2]  # Agent numbers
    width = 0.2  # Width of each bar
    x = np.arange(len(agents))  # X positions for agents

    # Plot bars for each scenario
    bars1 = ax.bar(x - 1.5*width, normal, width, label='Normal', color=colors[0], alpha=0.7)
    bars2 = ax.bar(x - 0.5*width, first_attack, width, label='Attack Agent 0', color=colors[1], alpha=0.7)
    bars3 = ax.bar(x + 0.5*width, second_attack, width, label='Attack Agent 1', color=colors[2], alpha=0.7)
    bars4 = ax.bar(x + 1.5*width, third_attack, width, label='Attack Agent 2', color=colors[0], alpha=0.7)

    # Customize the plot
    ax.set_xlabel('Agent Number')
    ax.set_ylabel('Taylor Expansion Error')
    ax.set_title('Taylor Expansion Error Comparison Across Scenarios')
    ax.set_xticks(x)
    ax.set_xticklabels([f'Agent {i}' for i in agents])
    ax.legend()
    ...
    plt.tight_layout()
    # Create the full file path with attack type
    attack_filename = f"taylor_error_comparison_{attack_type}.png"
    full_save_path = os.path.join(save_path, attack_filename)
    plt.savefig(full_save_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Taylor expansion error comparison plot saved to {save_path}")
...
def calculate_attack_reward(runner,attack_status=False,attacked_agent_id=None,seed=42,args=None,calculate_taylor=False):
    ...
    taylor_error_episode_mean.append([np.mean(result_list[j]) for j in range(runner.num_agents)])
...
def main():
    ...
    for seed in range(args.total_seeds):
        ...
        normal_reward,normal_taylor_episode_mean = calculate_attack_reward(runner,attack_status=False,attacked_agent_id=None,seed=args.seed,args=args,compute_taylor=True)
        print(f"Normal episode reward (no attack): {normal_reward:.6f}")
        attack_rewards = []
        
        
        attack_agent_zero_reward, attack_agent_zero_taylor_mean = calculate_attack_reward(runner,attack_status=True,attacked_agent_id=0,seed=args.seed,args=args,compute_taylor=True)
        print(f"Episode reward with Agent 0 attacked: {attack_agent_zero_reward:.6f}")
        attack_rewards.append((0, attack_agent_zero_reward))
        
        attack_agent_one_reward, attack_agent_one_taylor_mean = calculate_attack_reward(runner,attack_status=True,attacked_agent_id=1,seed=args.seed,args=args,compute_taylor=True)
        print(f"Episode reward with Agent 1 attacked: {attack_agent_one_reward:.6f}")
        
        final_attack_1_taylor_error.append(attack_agent_one_taylor_mean)
        attack_agent_two_reward, attack_agent_two_taylor_mean = calculate_attack_reward(runner,attack_status=True,attacked_agent_id=2,seed=args.seed,args=args,compute_taylor=True)
    ...
    # Calculate averages for Taylor errors
    print("\n" + "="*60)
    print("TAYLOR ERROR ANALYSIS")
    print("="*60)

    # Calculate mean for each agent and each scenario
    for i in range(3):
        normal_mean = np.mean(final_normal_taylor_error[i])
        
        attack_0_mean = np.mean(final_attack_0_taylor_error[i])
        
        attack_1_mean = np.mean(final_attack_1_taylor_error[i])
        
        attack_2_mean = np.mean(final_attack_2_taylor_error[i])
        
        print(f"\nAgent {i} Taylor Error Analysis:")
        print(f"  Normal scenario: {normal_mean:.6f}")
        print(f"  Attack Agent 0: {attack_0_mean:.6f}")
        print(f"  Attack Agent 1: {attack_1_mean:.6f}")
        print(f"  Attack Agent 2: {attack_2_mean:.6f}")
    ...
    # Save Taylor error analysis to CSV
    taylor_csv_filename = f"taylor_error_analysis_{timestamp}.csv"
    taylor_csv_path = os.path.join(args.output_dir, taylor_csv_filename)
