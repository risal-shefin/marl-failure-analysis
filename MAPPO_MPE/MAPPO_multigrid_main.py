...
class Runner_MAPPO_Multigrid:
    def __init__(self, args, env_name, number, seed):
        ...
        # Create env
        self.env = GymMultiGridWrapper.make_and_wrap_env(env_name, True)
        self.args.N = self.env.nagents  # The number of agents
        ...
        self.output_dir = os.path.join(args.output_dir, "train", f"{env_name}_{datetime.now().strftime('%Y%m%d-%H%M%S')}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--evaluate_freq", type=int, default=1000, help="Evaluate the policy every 'evaluate_freq' steps")
    ...
    runner = Runner_MAPPO_Multigrid(args, env_name=args.env_id, number=1, seed=42)
    runner.run()
