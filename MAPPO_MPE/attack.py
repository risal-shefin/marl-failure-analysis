...
def save_action_comparison_to_csv(unattacked_action_data, attacked_action_data, agent_id, csv_path):
    """
    Save action comparison data to a CSV file, without the original_action column.
    ...
    for entry in attacked_action_data:
        ...
        # Remove the original_action field from the data processing
    
    # Sort by step
    sorted_data = [combined_data[step] for step in sorted(combined_data.keys())]
    
    # Write to CSV with simplified columns
    with open(csv_path, 'w', newline='') as csvfile:
        # Remove original_action from fieldnames
        fieldnames = ['step', 'unattacked_action', 'attacked_action']
    ...
    print(f"Simplified action comparison saved to {csv_path}")
