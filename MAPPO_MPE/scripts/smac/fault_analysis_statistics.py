#!/usr/bin/env python3
"""Multi-seed fault analysis statistics for MAPPO agents in SMAC environments."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser("MAPPO Fault Analysis Statistics (SMAC)")
    parser.add_argument('--model_dir', type=str, required=True, help='Path to saved MAPPO model (.pt)')
    parser.add_argument('--map_name', type=str, required=True, help='SMAC map name (e.g., 3m, 8m, 3s_vs_4z)')
    ...
    # MAPPO runner hyperparameters (used for instantiating the runner)
...
class MultiSeedExperimentRunner:
    def __init__(self, config):
        self.config = config
        self.runner: Optional[Runner_MAPPO_SMAC] = None
        self.env = None
    ...
    def setup_experiment(self):
        ...
        self.logdir = os.path.join(cwd, 'runs', f"{self.config.map_name}_mappo_smac_{mode}_fault_stats", timestamp)
        os.makedirs(self.logdir, exist_ok=True)

        print(f"Log directory: {self.logdir}")

        self.runner = Runner_MAPPO_SMAC(self.config, self.config.map_name, number=1, seed=self.config.seed)
    ...
    def run_precomputation_mode(self):
        ...
        precompute_dir = os.path.join(cwd, 'data', 'precomputed_taylor_values',
                                     f"mappo_smac_{self.config.map_name}_nagents{self.runner.args.N}_{timestamp}")
