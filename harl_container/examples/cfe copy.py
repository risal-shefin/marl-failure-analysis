...
# """
# Full Counterfactual + Taylor/Frob evaluation for HARL (PettingZoo-MPE simple_spread_v2)
# - Uses ONLY actor.get_actions / actor.act (no evaluate_actions)
# - No try/except anywhere
# - Saves all outputs under ./results/<timestamp>/

# What it computes:
# 1) Baseline episode (deterministic actions) -> Taylor errors, Frobenius/2nd-order directional derivatives
# 2) Attacked episode (attacked agent forced to worst action at every step) -> same metrics
# 3) Per-timestep tot-ASE and Shapley values over non-attacked agents (coalition who can react after the forced action at t)

# CSV outputs:
# - happo_taylor_error_normal.csv / happo_taylor_error_atk_<id>.csv
# - happo_frobenius_norms_normal.csv / happo_frobenius_norms_atk_<id>.csv
# - happo_sec_dir_derivatives_normal.csv / happo_sec_dir_derivatives_atk_<id>.csv
# - totASE_per_t.csv
# - shapley_per_t.csv

# Plots:
# - plot_analysis_attacked_<id>.png            (Taylor)
# - plot_frobs_attacked_<id>.png               (Frobenius / negative-eig ratio)
# - plot_sec_dir_derivatives_attacked_<id>.png (2nd-order directional derivatives)
# - totASE_plot.png
# - shapley_plot.png
# """
...
#     mask,           # (1, 1)
#     avail_slice,    # (1, action_dim) or None
#     branch_seed,    # make deterministic per (t, agent)
#     N=256
# ):
#     n_act = discrete_n_actions(runner, agent_id)
#     allowed = available_action_indices(avail_slice, n_act)
...
#     for agent_id in range(runner.num_agents):
#         cur_obs = eval_obs[:, agent_id]
#         eval_actions, eval_actions_log_prob, _ = runner.actor[agent_id].get_actions(
#             cur_obs,
#             eval_rnn_states[:, agent_id],
#             eval_masks[:, agent_id],
...
#         # If you want Frobenius norm instead, comment the 4 lines above and use:
#         results.append(H.norm(p='fro').item())
#     return results
...
# def run_baseline_episode_and_record(runner, seed=23):
#     """Deterministic baseline episode — returns joint actions and (Taylor/Frob/2nd) metrics along the way."""
#     eval_obs, eval_share_obs, eval_avail = runner.eval_envs.reset(seed=seed)
...
#     while True:
#         # compute metrics BEFORE stepping (at current obs)
#         delta_errors = compute_taylor_policy(runner, eval_obs, eval_avail, rnn.copy())
#         frobs = compute_frob_norms(runner, eval_obs, 0, rnn_c, masks)
#         sdds = compute_2nd_ord_dir_derivatives(runner, eval_obs, 0, rnn_c, masks)
...
# # def run_attacked_episode_with_metrics(runner, attacked_id, seed=23):
# #     """Episode where the attacked agent executes the worst action at every step (others deterministic)."""
# #     eval_obs, eval_share_obs, eval_avail = runner.eval_envs.reset(seed=seed)
...
# #         # build actions: attacked gets worst; others deterministic
# #         actions_col = []
# #         for aid in range(n_agents):
# #             if aid == attacked_id:
# #                 derived_seed = seed * 100003 + t * 1009 + attacked_id * 101
...
# #     # Replay baseline to t-1
# #     for k in range(branch_t):
# #         obs, share_obs, rewards, dones, infos, avail = runner.eval_envs.step(baseline_actions[k])

# #     # At t: attacked forced to worst; others baseline
# #     actions_t = baseline_actions[branch_t].copy()
# #     derived_seed = seed * 100003 + branch_t * 1009 + attacked_id * 101
# #     worst_a = pick_worst_action_by_sampling(
# #         runner, attacked_id,
...
# #     # From t+1 onward: agents in S react; others (and attacked) follow baseline
# #     k = branch_t + 1
# #     while True:
# #         if np.all(dones):
# #             break
# #         actions_col = []
# #         for aid in range(n_agents):
# #             if (aid == attacked_id) or (aid not in coalition_allows_react):
# #                 if k < len(baseline_actions):
# #                     a_idx = int(baseline_actions[k][0, aid, 0])
# #                     a_np = np.array([[a_idx]], dtype=np.int64)
# #                     actions_col.append(a_np)
...
# # def tot_ase_and_shapley_at_t(runner, seed, baseline_actions, branch_t, attacked_id):
# #     all_non_attacked = [a for a in range(runner.num_agents) if a != attacked_id]
# #     cache = {}
...
# #         val = branch_rollout_value(
# #             runner=runner, seed=seed, baseline_actions=baseline_actions,
# #             branch_t=branch_t, attacked_id=attacked_id, coalition_allows_react=set(S_frozen),
# #         )
# #         cache[S_frozen] = val
# #         return val
...
# #                 v_S = v_of(S_f)
# #                 Sj = set(S); Sj.add(j)
# #                 v_Sj = v_of(frozenset(Sj))
# #                 w = factorial_ratio(m, n)
# #                 shap_dict[j] += w * (v_Sj - v_S)

# #     shapley_vec = [0.0] * runner.num_agents
# #     for aid, val in shap_dict.items():
# #         shapley_vec[aid] = float(val)

# #     return float(tot_ase), shapley_vec, float(v_all), float(v_empty)
# def tot_ase_and_shapley_at_t(runner, seed, baseline_actions, branch_t, attacked_id, min_window, max_window):
#     all_non_attacked = [a for a in range(runner.num_agents) if a != attacked_id]
#     cache = {}
#     def v_of(S_frozen):
...
#     "--save_dir",
#     type=str,
#     default="results",   # default to `results` per your earlier preference
#     help="Root folder where a timestamped subdir will be created."
# )
#     parser.add_argument("--min_window", type=int, default=0, help="inclusive attack start step")
#     parser.add_argument("--max_window", type=int, default=10, help="inclusive attack end step")
...
#     # 1) Baseline episode with metrics
#     (baseline_actions, base_return, T,
#      taylor_normal, frob_normal, sdd_normal) = run_baseline_episode_and_record(runner, seed=seed)
#     print(f"[Baseline] steps={T}, team_return={base_return:.3f}")

#     # 2) Attacked episode with metrics (worst action at each step)
#     (taylor_atk, frob_atk, sdd_atk, attacked_steps) = run_attacked_episode_with_metrics(runner, attacked_id, seed=seed)
...
#         totASE_series.append(tot_ase_t)
#         shapley_series.append(shap_vec_t)
#         print(f"[t={t:03d}] totASE={tot_ase_t:.6f} v_all={v_all:.3f} v_empty={v_empty:.3f}")
...
#     plot_frobs(frob_normal, frob_atk, attacked_steps, attacked_id, log_path)
#     plot_sec_dir_derivatives(sdd_normal, sdd_atk, attacked_steps, attacked_id, log_path)
#     # tot-ASE & shapley plots
#     plt_tot = os.path.join(log_path, "totASE_plot.png")
#     plt_shap = os.path.join(log_path, "shapley_plot.png")
#     plot_totase(totASE_series, plt_tot)
#     plot_shapley(shapley_series, plt_shap)
...
# # # cfe_eval_get_actions.py
# # """
# # Counterfactual Cascading-Effect Analysis for HARL (PettingZoo-MPE simple_spread_v2)
# # - Uses ONLY actor.get_actions(...) (no evaluate_actions)
# # - No try/except anywhere

# # What it computes per timestep t for a chosen attacked agent i:
# #   tot-ASE(t,i) = v(reactive) - v(muted)
# #   where "reactive" lets non-attacked agents react via their policies after the forced worst action at t,
# #   and "muted" forces everyone (including i) to follow the baseline recorded actions after t.

# # It also computes per-timestep Shapley values over the coalition of non-attacked agents.

# # Run (example):
# #   python cfe_eval_get_actions.py --algo happo --env pettingzoo_mpe --exp_name cfe_test \
# #     --attack_id 0 \
# #     --restore_dir /path/to/models --restore_reward -5.14 --restore_episode 2238 \
# #     --t_start 0 --t_end -1 --save_dir ./cfe_results
# # """
...
# # def factorial_ratio(m, n):
# #     # Shapley weight: m!(n-m-1)! / n!
# #     return (factorial(m) * factorial(n - m - 1)) / factorial(n)


# # def slice_avail(avail, aid):
# #     # Return (1, action_dim) or None
# #     if avail is None:
# #         return None
# #     first = avail[0]
# #     if first is None:
...
# # # ------------------------ Baseline Episode Recorder ------------------------

# # def run_baseline_episode_and_record(runner, seed=23):
# #     """
# #     One deterministic eval episode; record joint actions per step.

# #     Returns:
# #         actions_list: list of (1, n_agents, 1) int arrays
# #         total_return: sum of team rewards over episode (sum across agents per step)
# #         T: steps
# #     """
# #     eval_obs, eval_share_obs, eval_avail = runner.eval_envs.reset(seed=seed)
...
# #                 rnn[:, aid],
# #                 masks[:, aid],
# #                 slice_avail(eval_avail, aid),
# #                 deterministic=True,
# #             )
# #             rnn[:, aid] = _t2n(rnn_next)
# #             actions_col.append(_t2n(a))
# #         actions = np.array(actions_col).transpose(1, 0, 2)  # (1, n_agents, 1)
...
# #     return actions_list, total_return, len(actions_list)


# # # --------------------------- Worst Action via Sampling ---------------------------

# # def pick_worst_action_by_sampling(
# #     runner,
# #     agent_id,
# #     obs,            # shape (1, obs_dim)
# #     rnn_state,      # shape (1, rec_n, hid)
# #     mask,           # shape (1, 1)
# #     avail_slice,    # shape (1, action_dim) or None
# #     branch_seed,    # int for determinism per (t, agent)
# #     N=256
# # ):
# #     """
# #     Uses actor.get_actions(..., deterministic=False) N times and picks the least frequent *available* action.
# #     Restores RNG states after sampling to avoid side-effects.
# #     """
# #     n_act = discrete_n_actions(runner, agent_id)
# #     allowed = available_action_indices(avail_slice, n_act)

# #     # Save and set RNG states for determinism of sampling
# #     torch_state = torch.random.get_rng_state()
# #     np_state = np.random.get_state()
# #     torch.manual_seed(branch_seed)
# #     np.random.seed(branch_seed)
...
# #     counts = np.zeros(n_act, dtype=np.int64)
# #     for _ in range(N):
# #         # Important: do NOT mutate rnn_state; pass it as-is and ignore returned temp state
# #         a, logp, _ = runner.actor[agent_id].get_actions(
# #             obs,
# #             rnn_state,
# #             mask,
...
# #     # Choose least frequent among allowed actions
# #     counts_allowed = counts[allowed]
# #     argmin_local = int(np.argmin(counts_allowed))
# #     return int(allowed[argmin_local])


# # # ----------------------- Coalition Branch Rollout v(S) -----------------------
...
# # ):
# #     """
# #     Value v(S) for coalition S at time t:
# #       - Reset env with same seed
# #       - Replay baseline to t-1
# #       - At t: attacked forced to worst action; others baseline
# #       - After t: agents in S react via policy; others muted to baseline; attacked muted to baseline
# #     """
# #     obs, share_obs, avail = runner.eval_envs.reset(seed=seed)
...
# #     # 1) Replay baseline to t-1
# #     for k in range(branch_t):
# #         obs, share_obs, rewards, dones, infos, avail = runner.eval_envs.step(baseline_actions[k])

# #     # 2) At t: attacked takes worst; others take baseline
# #     actions_t = baseline_actions[branch_t].copy()

# #     # derived seed to make worst-action sampling deterministic per (t, agent)
# #     derived_seed = seed * 100003 + branch_t * 1009 + attacked_id * 101
...
# #         actions_col = []
# #         for aid in range(n_agents):
# #             # attacked muted after t; non-reacting agents muted to baseline
# #             if (aid == attacked_id) or (aid not in coalition_allows_react):
# #                 if k < len(baseline_actions):
# #                     a_idx = int(baseline_actions[k][0, aid, 0])
# #                     a_np = np.array([[a_idx]], dtype=np.int64)
...
# # # ------------------- tot-ASE and Per-timestep Shapley -------------------

# # def tot_ase_and_shapley_at_t(runner, seed, baseline_actions, branch_t, attacked_id):
# #     """
# #     tot-ASE(t) = v(U) - v(empty), where U = set of non-attacked agents who are allowed to react.
# #     Also returns per-agent Shapley among non-attacked agents.
# #     """
# #     all_non_attacked = [a for a in range(runner.num_agents) if a != attacked_id]
# #     cache = {}
...
# #             weights_only=False,
# #         )
# #         runner.actor[agent_id].actor.load_state_dict(sd_actor)

# #     if not runner.algo_args["render"]["use_render"]:
# #         sd_critic = torch.load(
# #             os.path.join(filepath, f"critic_agent_reward_{reward}_episode_{episode}.pt"),
# #             weights_only=False,
# #         )
# #         runner.critic.critic.load_state_dict(sd_critic)
...
# #                 weights_only=False,
# #             )
# #             runner.value_normalizer.load_state_dict(sd_vn)
...
# #     plt.xlabel("Timestep")
# #     plt.ylabel("tot-ASE")
# #     plt.title("tot-ASE per timestep")
...
# # def plot_shapley(shapley_matrix, save_path):
# #     arr = np.array(shapley_matrix)  # (T, n_agents)
# #     T, n_agents = arr.shape
# #     plt.figure(figsize=(12, 5))
# #     for aid in range(n_agents):
# #         plt.plot(range(T), arr[:, aid], linewidth=2, label=f"agent {aid}")
...
# #     plt.title("Per-timestep Shapley (non-attacked agents)")
# #     plt.legend()
# #     plt.grid(True, alpha=0.3)
# #     plt.tight_layout()
# #     plt.savefig(save_path, dpi=160)
...
# #     parser.add_argument("--save_dir", type=str, default="./cfe_results")
...
# #     args_cli, unparsed_args = parser.parse_known_args()
...
# #     keys = [k[2:] for k in unparsed_args[0::2]]
# #     values = [_process(v) for v in unparsed_args[1::2]]
# #     unparsed_dict = {k: v for k, v in zip(keys, values)}

# #     if args_cli.load_config != "":
# #         with open(args_cli.load_config, encoding="utf-8") as f:
# #             all_config = json.load(f)
# #         main_args = all_config["main_args"]
# #         algo_args = all_config["algo_args"]
# #         env_args = all_config["env_args"]
# #         main_args["exp_name"] = args_cli.exp_name
# #         main_args["algo"] = args_cli.algo or main_args["algo"]
# #         main_args["env"]  = args_cli.env  or main_args["env"]
# #     else:
# #         algo_args, env_args = get_defaults_yaml_args(args_cli.algo, args_cli.env)
# #         main_args = {
# #             "algo": args_cli.algo,
# #             "env": args_cli.env,
...
# #     # build runner
# #     set_all_seeds(args_cli.seed)
# #     runner = RUNNER_REGISTRY[main_args["algo"]](main_args, algo_args, env_args)

# #     if (args_cli.restore_dir != "") and (args_cli.restore_reward != "") and (args_cli.restore_episode != ""):
# #         restore(runner, args_cli.restore_reward, args_cli.restore_episode, args_cli.restore_dir)

# #     runner.prep_training()

# #     # Baseline
# #     actions_list, base_return, T = run_baseline_episode_and_record(runner, seed=args_cli.seed)
# #     print(f"[Baseline] steps={T}, team_return={base_return:.3f}")

# #     t0 = max(0, args_cli.t_start)
# #     t1 = (T - 1) if args_cli.t_end < 0 else min(args_cli.t_end, T - 1)
# #     timesteps = list(range(t0, t1 + 1))
# #     print(f"Analyzing timesteps {t0}..{t1} for attacked agent {args_cli.attack_id}")
...
# #             runner=runner,
# #             seed=args_cli.seed,
# #             baseline_actions=actions_list,
# #             branch_t=t,
# #             attacked_id=args_cli.attack_id,
# #         )
# #         totASE_series.append(tot_ase_t)
# #         shapley_series.append(shap_vec_t)
# #         print(f"[t={t:03d}] totASE={tot_ase_t:.6f}  v_all={v_all:.3f}  v_empty={v_empty:.3f}")

# #     save_root = os.path.abspath(args_cli.save_dir)
# #     ensure_dir(save_root)

# #     # Save CSVs
# #     tot_csv = os.path.join(save_root, "totASE_per_t.csv")
# #     with open(tot_csv, "w") as f:
# #         f.write("timestep,totASE\n")
# #         for i, val in enumerate(totASE_series):
# #             f.write(f"{timesteps[i]},{val}\n")
# #     print(f"Saved tot-ASE series -> {tot_csv}")

# #     shap_csv = os.path.join(save_root, "shapley_per_t.csv")
# #     with open(shap_csv, "w") as f:
# #         header = ["timestep"] + [f"agent_{i}" for i in range(runner.num_agents)]
# #         f.write(",".join(header) + "\n")
# #         for i, vec in enumerate(shapley_series):
# #             row = [str(timesteps[i])] + [f"{x:.6f}" for x in vec]
# #             f.write(",".join(row) + "\n")
# #     print(f"Saved per-timestep Shapley -> {shap_csv}")

# #     # Plots
# #     plot_totase(totASE_series, os.path.join(save_root, "totASE_plot.png"))
# #     plot_shapley(shapley_series, os.path.join(save_root, "shapley_plot.png"))
# #     print(f"Saved plots in {save_root}")
