...
def eval(runner, attack_status=False, attack_agent_id=0, enable_cfe_analysis=False):
    ...
    while True:
        ...
        if attack_status:
            # Use seeded random action for reproducibility
            np.random.seed(23)  # Different seed per timestep but reproducible
        ...
        if enable_cfe_analysis and attack_status:
            # Include ALL agents (including compromised agent for complete attribution)
            all_agents = list(range(runner.num_agents))
            
            # Compute agent-specific effects for all agents
            agent_effects = compute_agent_specific_effects(
                runner, eval_obs, eval_available_actions, eval_rnn_states_backup,
                attack_agent_id, all_agents
            )
...
def compute_agent_specific_effects(runner, eval_obs, eval_available_actions, eval_rnn_states, 
    ...
    """
    Compute agent-specific effects using observation similarity as a proxy.
    
    Since agents have independent policy networks with no gradient connections,
    we use cosine similarity between observations combined with magnitude differences
    as a measure of potential influence through environment interactions.
    
    Args:
        runner: HARL runner instance (kept for compatibility)
        eval_obs: Current observations tensor
        eval_available_actions: Available actions (unused but kept for compatibility)
        eval_rnn_states: RNN states (unused but kept for compatibility)
        compromised_agent_id: ID of the compromised agent
        target_agents_subset: List of agent IDs to analyze propagation through
        
    Returns:
        dict: Agent-specific effect values for each target agent based on observation similarity
    """
    eval_obs = torch.tensor(eval_obs, dtype=torch.float32)
    agent_effects = {}
    ...
    for target_agent in target_agents_subset:
        if target_agent == compromised_agent_id:
            # For the compromised agent itself, we measure its "self-effect"
            # This represents how much the attack changes the agent's own state/behavior
            compromised_obs = eval_obs[:, compromised_agent_id]
            compromised_obs_norm = torch.norm(compromised_obs)
            
            # Self-effect based on observation magnitude (higher magnitude = more effect)
            # This captures how much the compromised agent's observations deviate from "normal"
            if compromised_obs_norm > 0:
                # Normalized self-effect - could be enhanced with baseline comparison
                effect_magnitude = min(compromised_obs_norm.item() * 0.01, 0.1)  # Cap at 0.1 for stability
            else:
                effect_magnitude = 0.0
        else:
            # Use observation similarity as proxy for agent influence
            compromised_obs = eval_obs[:, compromised_agent_id]
            ...
            # Use cosine similarity for potential influence
            if compromised_obs_norm > 0 and target_obs_norm > 0:
                ...
                # Enhanced combine similarity with observation magnitude differences
...
def compute_marginal_contribution(runner, eval_obs, eval_available_actions, eval_rnn_states,
    ...
    print(f"Target effect with: {effect_with.get(target_agent, 0.0)}, without: {effect_without.get(target_agent, 0.0)}")
...
def compute_shapley_values(runner, eval_obs, eval_available_actions, eval_rnn_states, compromised_agent_id):
    """
    Compute Shapley values to attribute the effects to individual agents.
    ...
    Returns:
        dict: Shapley values for each agent (including the compromised agent)
    """
    n_agents = runner.num_agents
    all_agents = list(range(n_agents))  # Include ALL agents, including compromised one
    shapley_values = {}
    
    for target_agent in all_agents:
        marginal_contributions = []
        remaining_agents = [a for a in all_agents if a != target_agent]
...
def compute_per_agent_cascade_strength(normal_metrics, attacked_metrics, compromised_agent_id):
    """
    Compute per-agent cascade strength over time.
    Measures how much each individual agent's state deviates from normal due to attack.
    
    Args:
        normal_metrics: List of normal behavior metrics over time (each element is list of agent values)
        attacked_metrics: List of attacked behavior metrics over time
        compromised_agent_id: ID of compromised agent (to exclude from analysis)
        
    Returns:
        dict: Per-agent cascade strength timeline {agent_id: [strength_values_over_time]}
    """
    min_len = min(len(normal_metrics), len(attacked_metrics))
    n_agents = len(normal_metrics[0]) if normal_metrics else 0
    
    # Initialize per-agent cascade strength timelines
    agent_cascade_strengths = {}
    for agent_id in range(n_agents):
        if agent_id != compromised_agent_id:  # Exclude compromised agent
            agent_cascade_strengths[agent_id] = []
    
    # Compute per-agent cascade strength at each timestep
    for timestep in range(min_len):
        normal_state = normal_metrics[timestep]
        attacked_state = attacked_metrics[timestep]
        
        for agent_id in range(n_agents):
            if agent_id != compromised_agent_id:  # Exclude compromised agent
                # Per-agent deviation
                agent_deviation = abs(attacked_state[agent_id] - normal_state[agent_id])
                agent_cascade_strengths[agent_id].append(agent_deviation)
    
    return agent_cascade_strengths
...
def plot_counterfactual_decomposition(normal_data, attacked_data, agent_effects_timeline, 
    ...
    Args:
        ...
        agent_effects_timeline: Agent-specific effects over time
        shapley_timeline: Shapley values over time
        cascade_strength_timeline: Overall cascade strength over time
        compromised_agent_id: ID of compromised agent
    ...
    # Assign colors to all agents
    all_agent_ids = list(agent_effects_timeline[0].keys()) if agent_effects_timeline else []
    for i, agent_id in enumerate(sorted(all_agent_ids)):
        agent_colors[agent_id] = colors[i % len(colors)]
    
    # 1. Agent-specific effects over time (exclude attacked agent)
    ax1 = axes[0, 0]
    timesteps = range(len(agent_effects_timeline))
    for agent_id in agent_effects_timeline[0].keys():
        if agent_id != compromised_agent_id:  # Exclude the attacked agent from this plot
    ...
    if shapley_timeline:
        ...
        for agent_id in shapley_timeline[0].keys():
            ...
            ax2.plot(shapley_timesteps, shapley_vals, label=f'Agent {agent_id}', linewidth=2, 
                    linestyle='--', marker='o', color=agent_colors[agent_id])
    ...
    # 3. Overall cascade strength timeline
    ax3 = axes[1, 0]
    ax3.plot(timesteps, cascade_strength_timeline, 'r-', linewidth=3, label='Overall Cascade Strength')
    ax3.axvline(x=0, color='orange', linestyle=':', label='Attack Start', linewidth=2)
    ax3.set_title('Overall Temporal Cascade Strength')
    ax3.set_xlabel('Timestep') 
    ax3.set_ylabel('Cascade Magnitude')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    # 4. Per-agent cascade strength
    ax4 = axes[1, 1]
    # Compute per-agent cascade strengths
    per_agent_cascades = compute_per_agent_cascade_strength(normal_data, attacked_data, compromised_agent_id)
    
    # Plot per-agent cascade strength with consistent colors
    for agent_id, cascade_values in per_agent_cascades.items():
        ax4.plot(range(len(cascade_values)), cascade_values, label=f'Agent {agent_id}', 
                linewidth=2, marker='o', markersize=4, color=agent_colors[agent_id])
    
    ax4.axvline(x=0, color='orange', linestyle=':', label='Attack Start', linewidth=1, alpha=0.7)
    ax4.set_title('Per-Agent Cascade Strength')
    ax4.set_xlabel('Timestep')
    ax4.set_ylabel('Individual Agent Deviation')
    ax4.legend()
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="ID of the agent to attack.",
    )
